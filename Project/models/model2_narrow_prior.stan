...
model {
  mu_v ~ gamma(2, 2); // originally (3,1): mean = 3, SD = ~4.9
  mu_a ~ gamma(2, 2); // now mean = 1, SD = 0.7
  sigma_v ~ normal(2, 0.25); // originally (0,1)
  sigma_a ~ normal(2, 0.25);
  sigma_c ~ normal(2, 0.25);
  z_v_raw ~ std_normal();
  z_a_raw ~ std_normal();
  z_c_raw ~ std_normal();
